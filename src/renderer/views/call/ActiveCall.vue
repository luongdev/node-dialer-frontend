<template>
  <div class="flex justify-center items-center h-screen bg-gray-100">
    <a-card class="w-full bg-white overflow-hidden h-screen flex flex-col justify-center items-center">
      ...
      <div v-if="call.status?.length && 'TERMINATED' !== call.status" class="flex justify-center w-full px-6 mt-10">
        <a-button danger type="primary" size="large" @click="callHangup">
          Hangup
        </a-button>
      </div>

      <div v-if="call.status?.length && 'TERMINATED' !== call.status" class="flex justify-around w-full px-6 mt-8">
        <a-tooltip :title="call.mute ? 'Unmute' : 'Mute'">
          <a-button @click="call.toggleMute" type="default" size="large"
            class="bg-gray-200 hover:bg-gray-300 text-gray-600 text-lg p-2 mx-1">
        ...
        <a-tooltip :title="call.hold ? 'Unhold' : 'Hold'">
          <a-button @click="call.toggleHold" type="default" size="large"
            class="bg-gray-200 hover:bg-gray-300 text-gray-600 text-lg p-2 mx-1">
            <svg v-if="call.hold" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            ...
            <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21 7.5V18M15 7.5V18M3 16.811V8.69c0-.864.933-1.406
