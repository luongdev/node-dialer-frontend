<template>
  <div class="h-full w-full rounded-lg flex justify-between items-center py-2 px-4">
    <div class="flex flex-col justify-around">
      <div v-if="Answered !== call.status" class="text-lg font-bold">{{ call.status }}</div>
      <div v-else class="text-lg font-bold text-green-500 text-center">{{ duration }}</div>
      <div class="text-md text-gray-700">{{ phoneNumber }}</div>
      <div class="text-md text-gray-700">{{ phoneNumber }}</div>
    </div>
    <div class="flex flex-col justify-between items-center space-y-2 w-14">
      <a-tooltip :title="'Từ chối'" placement="right">
        <button @click="endCall" class="bg-red-500 text-white rounded-full w-10 h-10 flex justify-center items-center">
      ...
      <a-tooltip :title="'Nhận'" placement="right">
        <button v-show="!isCallActive" @click="answerCall" class="bg-green-500 text-white rounded-full w-10 h-10 flex justify-center items-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
      ...
      <div v-show="isCallActive" class="flex justify-center space-x-3 w-14">
        <a-tooltip :title="'mute'" placement="bottom">
          <button class="h-10 flex items-center">
            <svg v-if="true" height="24" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg">
        ...
        <a-tooltip :title="'hold'" placement="bottom">
          <button class="h-10 flex items-center">
...
import { useCallStore, CallStatus } from '@store/call/call';

const Answered = CallStatus.S_ANSWERED;
const Terminated = CallStatus.S_TERMINATED;
